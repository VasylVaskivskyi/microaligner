...
class DatasetStructCreator:
    def __init__(self):
        self._ref_ch = "DAPI"
        self.img_paths: Union[None, Path, List[Path], Dict[str, Dict[str, str]]] = None
    ...
    def _get_stack_builder_structure(
        self, cycle_map: Dict[str, Dict[str, str]]
    ) -> DatasetStruct:
        ...
        for cyc, ome_xml in ome_meta_dict.items():
            img_path_per_ch = cycle_map["Cycle " + str(cyc)]
            ...
            stack_builder_structure.ref_channel_ids[cyc] = ref_ch_ids[0]
            stack_builder_structure.ome_xmls[cyc] = ome_xml
        return stack_builder_structure

    def _get_stack_structure(self, img_path: Path) -> DatasetStruct:
        ...
        for cyc in range(0, ncycles):
            ...
            stack_structure.ref_channel_ids[cyc] = ref_ch_position_in_cyc
            stack_structure.ome_xmls[cyc] = ome_xml
        return stack_structure

    def _get_img_list_structure(self, img_paths: List[Path]) -> DatasetStruct:
        img_list_structure = DatasetStruct()

        for cyc, img_path in enumerate(img_paths):
            ome_xml = read_ome_meta_from_file(img_path)
            ome_info = collect_info_from_ome(self._ref_ch, ome_xml)
            ref_ch_ids = ome_info["ref_ch_ids"]
            nchannels = ome_info["nchannels"]
            ...
            img_list_structure.ref_channel_ids[cyc] = ref_ch_ids[0]
